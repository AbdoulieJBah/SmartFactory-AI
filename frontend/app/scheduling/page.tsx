...
export default function SchedulingPage() {
  ...
  async function loadData() {
    try {
      ...
      const [scheduleRes, ordersRes, workCentersRes] = await Promise.allSettled([
        api.get("/scheduling/"),
    ...
    <div className="min-h-screen bg-[#f6f8fb]">
      ...
      <main className="ml-72 min-h-screen p-6">
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Real scheduling from backend database.
            </p>
          </div>
        ...
        <section className="mb-5 rounded-xl border bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold">Create Schedule</h2>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, capacity_load: Number(e.target.value) })}
        ...
        {loading ? (
          ...
          <div className="rounded-xl border bg-white p-8 text-gray-500">
            No schedules found. Create your first production schedule.
          </div>
        ) : (
                  ...
                  {items.map((item) => (
                    <div key={item.id} className="rounded-xl border p-4">
                      <div className="mb-2 flex justify-between gap-3">
                        ...
                        <span className={`h-fit rounded-full px-3 py-1 text-xs font-semibold ${priorityClass(item.priority)}`}>
                          {item.priority}
                        </span>
                      </div>
